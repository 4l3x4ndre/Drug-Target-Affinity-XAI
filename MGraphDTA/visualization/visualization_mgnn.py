...
class GradAAM():
    ...
    def __call__(self, data):
        ...
        grad = torch.autograd.grad(output, self.target_feat)[0] # grad returns only one element
        channel_weight = torch.mean(grad, dim=0, keepdim=True)
        channel_weight = normalize(channel_weight) 
        # channel_weight: torch.Size([1, 228])
        # self.target_feat: size depends on the target: torch.Size([X, 228])
    ...
    def forward_features(self, data):
        ...
        output, protein_f, ligand_f = self.model.forward_features(data)
        ...
        return output.detach().cpu().numpy(), protein_f, ligand_f, cam
...
def main():
    ...
    try:
        ...
        model_dict = torch.load('pretrained_model/epoch-173, loss-0.1468, cindex-0.8998, test_loss-0.1750.pt', weights_only=True)
    ...
    print("Saving results to:", os.path.abspath("results"))
