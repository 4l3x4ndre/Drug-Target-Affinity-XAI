...
class TargetRepresentation(nn.Module):
    ...
    def forward_from_embedding(self, emb):
        """
        Forward pass starting from precomputed embeddings.
        emb: torch.Tensor of shape [B, L, embedding_num]
             (same as self.embed(x) output, but before permute)
        """
        # replicate the same path after embedding
        x = emb.permute(0, 2, 1)          # [B, embedding_num, L]
        feats = [block(x) for block in self.block_list]
        x = torch.cat(feats, -1)
        x = self.linear(x)
        return x
