...
from GradCam.pytorch_grad_cam.activations_and_gradients import ActivationsAndGradients
from GradCam.pytorch_grad_cam.utils.image import scale_cam_image
from GradCam.pytorch_grad_cam.utils.model_targets import ClassifierOutputTarget
from GradCam.pytorch_grad_cam.utils.svd_on_activations import get_2d_projection
...
class BaseCAM:
        ...
        if self.uses_gradients:
            ...
            if self.detach:
                try:
                    ...
                    print("Got error:", e)
                    print("  => using loss.mean().backward() instead")
    ...
    def get_target_width_height(self, input_tensor: torch.Tensor) -> Tuple[int, int]:
        if len(input_tensor.shape) == 4:
            ...
            raise ValueError(f"Invalid input_tensor shape. Only 2D or 3D images are supported. Got input_tensor.shape={input_tensor.shape}")
